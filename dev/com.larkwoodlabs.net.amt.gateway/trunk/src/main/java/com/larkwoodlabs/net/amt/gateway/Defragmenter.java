 ...
 * 
 * File: Defragmenter.java (com.larkwoodlabs.net.amt.gateway)
 * 
...
/**
 * An {@link OutputChannel} that reassembles fragmented IP datagrams.<p>
 * This channel accepts {@link IPPacket} objects that carry complete 
 * or fragmented datagrams. If a packet carries a complete datagram, the defragmenter
 * will immediately send the packet to the downstream output channel. 
 * If the packet carries a datagram fragment, the fragmenter will attempt to combine
 * that packet with other incoming datagram fragments to reconstruct the original datagram.
 * Once the datagram is complete, the defragmenter will send a new packet carrying
 * the completed datagram to the downstream channel.<p>
 * The defragmenter imposes a 60 second time limit on for reassembly of a datagram.
 * If a particular datagram cannot be reassembled within timeout period, the partial
 * reconstruction of that datagram is flushed from the internal cache. (Note: The reassembly
 * process will restart if additional fragments for the datagram arrive after the time-limit.)<p>
 * The defragmenter can be attached to a second output channel that can be used
 * to receive a timeout notification. The defragmenter will send the packet containing
 * fragment zero of the datagram if that packet is available and the destination address
 * is not a multicast address. No timeout is reported for a multicast packets<p>
 * The defragmenter must allocate a 64K buffer to reassemble each fragmented datagram.
 * If a large number of fragmented datagrams arrive within a 60s time window, the
 * defragmenter will consume a significant amount of memory. To avoid excessive memory use,
 * the defragmenter can be constructed with a limit on the number of datagrams that can
 * undergo simultaneous reassembly. If the limit is reached, the defragmenter places the
 * new datagram fragments into a queue for future reassembly. This queue is limited by
...
public final class Defragmenter 
    ...
    final static class ReassemblyBuffer {
        ...
        public static final Logger logger = Logger.getLogger(Defragmenter.ReassemblyBuffer.class.getName());

        static final int EOL = 0xFFFF;
        ...
        long timestamp;
        long maxAge;
        IPPacket fragmentZeroPacket;
        
        final String ObjectId = Logging.identify(this);

        public ReassemblyBuffer(final ByteBuffer identifier, final long maxAge) {
        ...
        public boolean isExpired(final long currentTimeMs) {
        ...
        public long getAge(final long currentTimeMs) {
            return currentTimeMs - timestamp;
        }

        public boolean addFragment(final IPPacket packet) throws ParseException {
        ...
        public IPPacket getFragmentZeroPacket() {
            return this.fragmentZeroPacket;
        }

        public IPPacket getCompletedPacket() {
            if (isComplete()) {
                return this.fragmentZeroPacket;
            }
            return null;
        ...
        final void setHole(final int hole,
                           final int holeLast,
                           final int prevHole,
                           final int nextHole) {
            if (logger.isLoggable(Level.FINER)) {
                logger.finer(ObjectId + " new hole.first=" + hole + " hole.last=" + holeLast + " hole.prev=" + prevHole + " hole.next=" + nextHole);
            }
        ...
        final void setOffsetValue(final int byteOffset, int offset) {
            buffer[byteOffset] = (byte)((offset >> 8) & 0xFF);
            buffer[byteOffset + 1] = (byte)(offset & 0xFF);
        }

        final int getOffsetValue(final int byteOffset) {
            return ((buffer[byteOffset] & 0xFF) << 8) | (buffer[byteOffset + 1] & 0xFF);
        }

        final int getHoleLast(final int holeFirst) {
            return getOffsetValue(holeFirst*8);
        }

        final void setHoleLast(final int holeFirst, final int holeLast) {
            setOffsetValue(holeFirst*8, holeLast);
        }

        final int getPrevHole(final int holeFirst) {
            return getOffsetValue(holeFirst*8 + 2);
        }

        final void setPrevHole(final int holeFirst, final int prevHole) {
            setOffsetValue(holeFirst*8 + 4, prevHole);
        }

        final int getNextHole(final int holeFirst) {
            return getOffsetValue(holeFirst*8 + 4);
        }

        final void setNextHole(final int holeFirst, final int nextHole) {
            setOffsetValue(holeFirst*8 + 6, nextHole);
        }
        
        /**
         * Algorithm described in <a href="http://tools.ietf.org/html/rfc815">[RFC-851]</a><p>
         * This will remove all of the hole descriptors that will be overwritten by the fragment.
         * 
         *  1. Select the next hole  descriptor  from  the  hole  descriptor
         *     list.  If there are no more entries, go to step eight.<p>
         *  2. If fragment.first is greater than hole.last, go to step one.<p>
         *  3. If fragment.last is less than hole.first, go to step one.<p>
         *      <blockquote>
         ...
         * 
         * @param buffer - The IP datagram fragment.
         * @param fragmentOffset - The fragment offset.
         * @param isMoreFragments - Indicates this is the end fragment.
         * @return Indicates whether the datagram is complete.
         */
        final boolean addFragment(final ByteBuffer buffer, final int fragmentOffset, final boolean isMoreFragments) {
            ...
            while (hole != EOL) {
                ...
                if (fragmentFirst > hole) {
                    if (logger.isLoggable(Level.FINER)) {
                        logger.finer(ObjectId + " hole.first=" + hole + " hole.last=" + holeLast + " frag.first=" + fragmentFirst + " frag.last=" + fragmentLast + " <-- fragment overlaps end of hole");
                    }
                    setHoleLast(hole, fragmentLast - 1);
                }
                else if (fragmentLast < holeLast && isMoreFragments) {
                    logger.finer(ObjectId + " hole.first=" + hole + " hole.last=" + holeLast + " frag.first=" + fragmentFirst + " frag.last=" + fragmentLast + " <-- fragment overlaps start of hole");
                    ...
                    logger.finer(ObjectId + " hole.first=" + hole + " hole.last=" + holeLast + " frag.first=" + fragmentFirst + " frag.last=" + fragmentLast + " <-- fragment overlaps entire hole - delete hole");
                    if (prevHole == EOL) {
    ...
    public static final Logger logger = Logger.getLogger(Defragmenter.class.getName());
     ...
     * packet header. A <code>ByteBuffer</code> provides the
     * <code>hashCode()</code> and <code>equals()</code> methods that are
     * required to locate a {@link ReassemblyBuffer} in the {@link #cache}
     * HashMap.
     * 
     * @param packet
     ...
     *            the identifier.
     */
    public static ByteBuffer constructBufferIdentifier(final IPv6Packet packet) {
     ...
     *            bytes).
     */
    public static ByteBuffer constructBufferIdentifier(final byte[] sourceAddress,
     ...
     *            the identifier.
     */
    public static ByteBuffer constructBufferIdentifier(final IPv4Packet packet) {
     ...
     * @param outputChannel - The output channel that will receive reassembled datagrams. Required.
     * @param timeoutChannel - The optional output channel used to receive timeout notifications.
     *                         May be <code>null</code>
     * @param maxCacheSize - Maximum number of datagrams that can be reassembled at the same time.
     *                       A value of zero is used to indicate that there should be no limit.
     * @param taskTimer - Externally constructed Timer used to execute the task used to check timeouts.
     *                    The defragmenter will construct its own timer if this value is <code>null</code>.
     */
    public Defragmenter(final OutputChannel<IPPacket> outputChannel,
        ...
        this.taskTimer = (taskTimer != null ? taskTimer : new Timer(Defragmenter.class.getName()));
    }
    ...
    public void send(final IPPacket packet, final int milliseconds) throws IOException, InterruptedIOException, InterruptedException {
        ...
        if (logger.isLoggable(Level.FINER)) {
            logger.finer(Logging.entering(ObjectId, "Defragmenter.send", packet, milliseconds));
        }
    ...
    private Result processPacket(final IPPacket packet, final int milliseconds) throws InterruptedIOException, IOException, InterruptedException {
        ...
        if (reassemblyBuffer == null) {
            ...
            if (this.maxCacheSize != 0 && this.cache.size() >= this.maxCacheSize) {
                if (logger.isLoggable(Level.FINER)) {
                    logger.finer(ObjectId + " reassembly cache is full cache-size=" + this.cache.size() + " queue-size=" + this.pendingQueue.size());
                }
        ...
        synchronized (this.lock) {
            ...
            if (reapAll) {
                ...
                while (iter.hasNext()) {
                    ...
                    // The buffer will no longer exist if flushed by the processPendingPackets() call below.
                    ReassemblyBuffer buffer = this.cache.get(iter.next());
                    if (buffer != null && buffer.isExpired(currentTimeMillis)) {

                        if (logger.isLoggable(Level.FINER)) {
                            logger.finer(ObjectId +
                                         " reassembly timeout exceeded buffer-identifier=" + Logging.identify(buffer.getIdentifier()) +
                        ...
                        // Remove the reassembly buffer from the cache now that we're done with it.
                        iter.remove();
                ...
                // Now that the cache size is reduced, we can process these to create a new entries
                if (this.maxCacheSize > 0 && this.cache.size() < this.maxCacheSize) {
