...
public final class RtspMulticastRelayApplet extends Applet {

    /*-- Static Variables ----------------------------------------------------*/

    public static final Logger logger = Logger.getLogger(RtspMulticastRelayApplet.class.getName());
    ...
    public RtspMulticastRelayApplet() {
        setLoggerLevels();
    }

    public String getAppletInfo() {
        return "RTSP Multicast Relay Applet, Version 0.0.1.x";
    }
    ...
    public void init() {
        ...
        logger.finer(Logging.entering(ObjectId, "RtspMulticastRelayApplet.init"));
        ...
        if (this.isJavaScriptEnabled) {
            ...
            logger.finer(ObjectId + " onError=" + this.onErrorFuncName);
    ...
    public void start() {
        ...
        logger.finer(Logging.entering(ObjectId, "RtspMulticastRelayApplet.start"));

        if (this.server == null) {
            fireOnError("applet failed to start");
            return;
        }
        ...
        try {
            ...
            String message = "applet failed to start - " + e.getClass().getName() + ":" + e.getMessage();
    ...
    public void stop() {
        ...
        logger.finer(Logging.entering(ObjectId, "RtspMulticastRelayApplet.stop"));
    ...
    @Override
    public void destroy() {
        logger.finer(Logging.entering(ObjectId, "RtspMulticastRelayApplet.destroy"));
    }
    
    private boolean startServer() throws IOException {

        logger.finer(Logging.entering(ObjectId, "RtspMulticastRelayApplet.startAsPrimary"));

        try {
            this.server.start();
            setLoggerLevels();
            logger.info(ObjectId + " applet started relay instance");
            return true;
         }
             ...
             logger.info(ObjectId + " applet cannot start relay instance because one is already running");
             return false;
         }
    }
    ...
    private void waitForServer() throws IOException {
        ...
        logger.finer(Logging.entering(ObjectId, "RtspMulticastRelayApplet.waitForPrimary"));

        socket.connect(new InetSocketAddress(InetAddress.getByName("127.0.0.1"),8054));
        try {

            logger.finer(ObjectId + " waiting for preexisting relay to shutdown");
